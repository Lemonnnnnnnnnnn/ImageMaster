...
<div class="px-5 py-2 bg-gray-700 flex justify-between items-center">
  ...
  <div class="flex gap-2 min-w-60">
    <Button on:click={MangaService.toggleNavigation} variant="filled" color="primary">
      {showNavigation ? '隐藏导航' : '显示导航'}
    </Button>
    <Button on:click={MangaService.deleteAndViewNextManga} variant="filled" color="error">删除并看下一部</Button>
  </div>
</div>